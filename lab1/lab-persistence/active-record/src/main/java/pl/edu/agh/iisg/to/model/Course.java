...
public class Course {
    ...
    public boolean enrollStudent(final Student student) {
        ...
        String checkIfExistsSql = "SELECT * FROM student_course WHERE student_id = (?)";
        Object[] args0 = {student.id()};
        Object[] args = {
            student.id(),
            this.id
        };
        ...
        try {
            ResultSet rs = QueryExecutor.read(checkIfExistsSql, args0);
            if(!rs.next()){
    ...
    public List<Student> cachedStudentsList() {
        return isStudentsListDownloaded ? enrolledStudents : (enrolledStudents = studentList());
    }
