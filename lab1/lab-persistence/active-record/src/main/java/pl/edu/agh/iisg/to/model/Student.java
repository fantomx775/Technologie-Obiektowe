...
public class Student {
    ...
    public Map<Course, Float> createReport() {
        ...
        String sqlGrade = "SELECT * FROM grade WHERE student_id = (?)";
        Object[] args = {this.id};
        Map<Course, Float> report = new HashMap<>();
        try{
            ResultSet rs = QueryExecutor.read(sqlGrade, args);
            int lastCourse = rs.getInt("course_id");
            int currentCourse;
            float sum = 0.0F;
            int count = 0;
            while(rs.next()){
                currentCourse = rs.getInt("course_id");
                if(currentCourse != lastCourse){
                    Optional<Course> courseOptional = Course.findById(lastCourse);
                    Course course = courseOptional.orElseThrow(() -> new NoSuchElementException("Course not found"));
                    report.put(course, sum/count);
                    sum = 0.0F;
                    count = 0;
                    lastCourse = currentCourse;
                }
                sum += rs.getFloat("grade");
                count++;
            }
            Optional<Course> courseOptional = Course.findById(lastCourse);
            Course course = courseOptional.orElse(new Course(0, ""));
            report.put(course, sum/count);
        ...
        return report.isEmpty() ? Collections.emptyMap() : report;
    }
